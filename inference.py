...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance == None):
            allPossible[self.getJailPosition()] = 1.0
            self.particle = allPossible
        else:
            bf = self.getBeliefDistribution()
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[trueDistance] * bf[p]
            sum = 0
            for pp in allPossible:
                sum+=allPossible[pp]
            if sum == 0:
                self.initializeUniformly(gameState)
            else:
                ...
                for i in range(0,self.numParticles):
                    res.append(util.sample(allPossible))
                    self.particle = res
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        random.shuffle(pos)
        #num = self.numParticles / len(pos)
        self.particles = []
        n = self.numParticles
        while n >= len(pos):
            self.particles += pos
            n -= len(pos)
        self.particles += pos[0: n - 1]
